...
const News = ()=>{
    const itemsAry = [{
        ...
        list: ['urgent transport solutions','reliable & experienced staffs','real time tracking','custom duty']
    }]

    const items = itemsAry.map((item,idx)=><div key={idx} className='flex  gap-4 border-y py-4 group'>
    <div className=' w-3/4 relative'>
        ...
        <img className='h-full object-cover' src={item.img} alt={`news${idx}`} />
    </div>
    <div className=' text-center'>
        <img src={cal} alt="calender" />
        <p style={{color: color.darkTxt}} className=' font-bold text-2xl'>{item.date}</p>
        <p style={{color: color.grayTxt}} className=' capitalize text-sm'>march</p>
    </div>
    <div className=' font-["Rubik"]  border-l border-[#d6d6d6] px-4'>
        <p className=' font-bold text-[#1c1f35] text-lg group-hover:text-[#ffbe34]'>{item.info}</p>
        <p className=' text-[#666c89]'>{item.txt}</p>
        <ul className=' list-inside list-disc '>
            {item.list.map((itm,idx)=><li key={idx}>{itm}</li>)}
        </ul>
    </div>
</div>)
    return(
