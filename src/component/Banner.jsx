...
const Banner = ({img,txt,txt2,extra,info,btn_txt}) => {
    return (
        ...
        //     </div>
        // </div>
        <div className='bg-red-400 bg-no-repeat bg-cover bg-center relative h-[70vh]' style={{ backgroundImage: `url(${img})` }}>
            <div className=' absolute top-2/4 -translate-y-2/4'>
                {/* linear-gradient(94.06deg, #FFB629 -1.21%, #FFDA56 58.66%, #FFD7A6 116.84%) */}
                ...
                <p className=' font-[Krub] font-medium w-[35%]'>{txt2}</p>
                <button style={{ background: `linear-gradient(94.06deg, ${color.grdt} -1.21%, ${color.grdt2} 58.66%, ${color.grdt3} 116.84%)` }} className='p-4 text-black capitalize font-medium my-4'>{btn_txt}</button>
            </div>
        </div>
    )
}
