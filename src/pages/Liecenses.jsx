...
const Liecenses = ()=>{
    ...
    const item = itemAry.map((item,idx)=>(
        <div key={idx} className='bg-red-500 flex items-start justify-between'>
                <p style={{color: color.darkTxt}} className='bg-red-200 font-semibold text-2xl capitalize'>{item.txt}</p>
                <div className='bg-red-300 w-4/5 flex flex-col gap-4'>
                    <p style={{color: color.grayTxt}}>
                    {item.txt2}
                    </p>
                    {item.txt3&&<p style={{color: color.darkTxt}} className=' font-bold capitalize bg-red-500 w-fit flex items-center gap-2 text-xl'>{item.txt3}:<span className=' font-normal bg-red-400 '>Liecenses</span></p>}
    ...
    return(
        <div>
            ...
            <div className='bg-blue-500 w-4/5 mx-auto px-4 space-y-4 my-8'>
                {item}
            </div>
            <Footer/>
        </div>
